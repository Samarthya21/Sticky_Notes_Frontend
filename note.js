...
export default function Note({title,content}) {
  ...
  return (
    <div className="m-2 h-96 w-96 border border-8 border-double border-black rounded-lg shadow-lg p-4">
      <form className="h-full w-full">
        <label className="block text-xl font-semibold text-gray-700 mb-4">Enter your title:</label>
        <input
          type="text"
          className="w-full h-10 border border-4 border-purple-600 rounded-md p-2 focus:outline-none focus:ring focus:border-purple-600"
          placeholder="Title"
          value={title_of}
          onChange={(e) => setTitle(e.target.value)}
        />
        <label className="block text-xl font-semibold text-gray-700 mb-4 mt-4">Enter your content:</label>
        <textarea
